...
/// 首页：快速记账 + 今日摘要 + 上次作业详情。
///
/// 本次升级只重排/重绘 UI 层，未改动任何 Provider 读写方式、状态形状或
/// 业务规则（作业类型可见性规则、单价来源、汇总口径等与原版逐条一致）。
        ...
        children: [
          ...
          Padding(
            ...
            child: TextFormField(
              ...
              maxLines: 2,
              onChanged: (v) =>
                  ref.read(selectedDateRecordProvider.notifier).updateRemark(v),
            ),
          ),
                ...
                child: Column(
                  ...
                  children: allTypes.map((type) {
                    ...
                    return CheckboxListTile(
                      title: Text(type),
                      subtitle: isAdvanced ? const Text('其他作业类型') : null,
                      value: checked,
                      onChanged: (v) {
                        if (isAdvanced) {
...
class _SummaryCards extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final dayQty = dayRecords
        ...
        .fold<int>(
            0, (s, r) => s + r.jobQuantities.values.fold(0, (a, b) => a + b));
    ...
    final yestAmount =
        yestRecords.fold<double>(0, (s, r) => s + r.amount(unitPrices));
        ...
        children: [
          ...
          Expanded(
            child: StatCard(
              ...
              subtitle: '昨日 $yestQty车 · ¥${yestAmount.toStringAsFixed(2)}',
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    record.workerName.isEmpty ? '（未填写姓名）' : record.workerName,
